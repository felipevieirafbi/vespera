...
import { WorldObstacle } from '../types/game';
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* 300 Procedural Obstacle dots colored by biome */}
        {obstacles.map((obs) => {
          const pos = toRadar(obs.x, obs.y);
          return (
            <div
              key={`radar-obs-${obs.id}`}
        ...
        {/* Player Blip */}
        <div
          className="absolute w-3 h-3 -ml-1.5 -mt-1.5 rounded-sm bg-cyan-400 shadow-[0_0_8px_#00FFFF] border border-white transition-all duration-75 z-10"
